...
## AI concept layout (added 2026-05-31)

When a submitted brief has **no equivalent standard build**, the system can generate a rough **2D concept layout** (zones: entry, kitchenette, bathroom, storage, sleeping) as a starting point for review. It is **optional to generate** and **must be approved before the project can go to production (build)** — that approval is the gate.

- **"No equivalent build"** is decided by `src/lib/standardBuilds.ts` → `findEquivalentBuild(brief)` (matches length + sleeps + wet-bath + kitchenette; ignores solar/battery upgrades). Catalog is `STANDARD_BUILDS`.
- **Domain:** `ConceptLayout` (status `pending_review`/`approved`/`rejected`, `source` `ai`/`template`, `zones: LayoutZone[]` in feet within a length×width envelope) on `Project.conceptLayout`. Types in `src/types/index.ts`; geometry + validation in `src/lib/conceptLayout.ts` (`envelopeFor`, `templateLayout`, `validateLayout`, `REQUIRED_ZONES`).
- **Generator behind the data seam** (`src/data/conceptLayoutGenerator.ts`): `ConceptLayoutGenerator` interface with **`ClaudeConceptLayoutGenerator`** (Claude API → structured JSON via `output_config.format` json_schema → validated against the envelope; model `claude-opus-4-8`, adaptive thinking, cached system prompt) and a deterministic **`TemplateConceptLayoutGenerator`** fallback. Selection is in `src/data/index.ts`: uses the AI generator when `VITE_ANTHROPIC_API_KEY` is set (browser key + `dangerouslyAllowBrowser` — acceptable for this FE-only prototype; moves server-side in the Next.js/Supabase target), else the template. The Claude generator also falls back to the template on any error/invalid output, so the feature always works offline and in tests.
- **Repository methods** (`ProjectRepository`): `findEquivalentBuild`, `generateConceptLayout` (throws if an equivalent build exists), `approveConceptLayout` (the production gate), `rejectConceptLayout`.
- **UI:** `src/components/ConceptLayoutDiagram.tsx` (accessible SVG — `role="img"` + title/desc, every zone labeled so colour isn't the only signal) and `ConceptLayoutSection.tsx` (three states: matches-a-build / generate / review+approve), wired into `ProjectView`. Decision recorded: chose the Claude-API generator over 3rd-party floorplan APIs (Maket.ai etc. are house-oriented, heavier, and overkill for tiny trailers).
- **Why Claude API not a 3rd-party floorplan service:** researched Maket.ai / Giraffe360 / getfloorplan / ideal.house — all tuned for full houses, need keys/cost, heavier integration. Claude→JSON→SVG fits the data-layer seam, honours the 16–18ft envelope, and is fully controllable.

To work with Claude in this repo, the **claude-api** skill is the source of truth (SDK `@anthropic-ai/sdk`, `messages.create` + `output_config.format`, adaptive thinking, prompt caching).
...
**Logo (2026-05-30):** `src/components/Logo.tsx` ports the Make redesign's `Logo` (mountain-over-trailer mark; `variant` default/white/mark, `size` sm/md/lg; wordmark = bold "CampAway" + normal "Design"). `AppNav` shows the `mark` in a white card on the left with the nav on the right (matches the Make `AppNav` layout). Accessibility added: `mark` variant carries `role="img"`/`aria-label`; the full variant's SVG is `aria-hidden` (visible wordmark is the name). The ProjectList hero renders `<Logo size="lg" />` inside the page `<h1>` (matches the Make hero, keeps the heading outline). Also ported the Make `LogoShowcase` brand-reference page at route `/logo` (`src/routes/LogoShowcase/`).

**Re-checked the Make screens (2026-05-30):** the five `Modern*` screens are unchanged from the original port — our ported route versions are strict supersets (repository-wired, validated, accessible), so they were left as-is rather than regressed to the hardcoded Make code. The only net-new generated code was the Logo system + LogoShowcase (now integrated).

**Verified green:** `npx tsc --noEmit`, `npx eslint .`, `npm run build`, and `npx vitest run --project unit` (now 35 tests) all pass as of 2026-05-30.
